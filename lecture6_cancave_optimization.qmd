...
$$
F_x(x^a) \geq \frac{F(x^b) - F(x^a)}{x^b - x^a}
$$
...
**凹规划**是一类特殊的数学优化问题，其目标函数为**凹函数**，且约束条件定义的可行域为**凸集**。具体形式如下：
...
- **目标函数** $f(x)$ 是**凹函数**（若为最小化问题，则要求 $f(x)$ 是凸函数）。

- **不等式约束** $g_i(x)$ 是**凹函数**（确保可行域 $\{x \mid g_i(x) \geq 0\}$ 是凸集）。

- **等式约束** $h_j(x)$ 是**仿射函数**（即线性函数加常数，形如 $h_j(x) = a_j^T x + b_j$）。
